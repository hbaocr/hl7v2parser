...
class HL7Message {
  ...
  parse(buf, options) {
    ...
    try {
      const dels = this.delimitersFromMSH(str);
      this._delimiters = dels;
    } catch (e) {
      /* istanbul ignore next */
      throw new ParseError('Invalid message header');
    }
    
    const lines = this.segmentize(str);
    for (const [i, line] of lines.entries()) {
      if (!line)
        continue;
      //console.log(`Segmenet ${i}: ${line}`);
      const segment = new HL7Segment(this, {customDict}, this._delimiters);
      try {
        segment.parse(line);
      } catch (e) {
        /* istanbul ignore next */
...
//split the message into segments by \r or \n or \r\n
segmentize(message) {
  const regex = new RegExp(`${CR}?${LF}|${CR}`);
 ...
 */
delimitersFromMSH(message) {
  const delimiters = [];
  const msh = this.segmentize(message)[0];
  // MSH|^~\&|...
  delimiters.push(msh[3]); //field
  delimiters.push(msh[5]); //repitition
  delimiters.push(msh[4]); //component
  delimiters.push(msh[7]); //subcomponent
  delimiters.push(msh[6]); //escape
  ...
  /**
   * Converts the segments of an HL7 message into a more readable format.
   * @return {JSON} - An array of objects representing each segment in the HL7 message.
   *                       Each object has the segment type as the key and the result of calling the `view` method on the segment as the value.
   */
  view() {
    const out = {};
