...
class HL7Field {
   ...
   */
  constructor(segment, name, def, customDict, delimiters) {
  ...
  add() {
    ...
    const data = new HL7FieldData(this, this._def, undefined, this._customDict, this._delimiters);
  ...
  parse(value) {
    ...
    if (this.segment.type === 'MSH' && this.sequence <= 2)
      ...
      const values = value.split(this._delimiters[1]);
  ...
  toHL7() {
    ...
    for (const data of this._data)
      out += (out ? this._delimiters[1] : '') + data.toHL7();
    return out;
  }
  view() {
    const fieldData = this._data;
    if (fieldData) {
      if (fieldData.length === 1) {
        return fieldData[0].view();
      } else {
        const out = fieldData.map(data => data.view());
        return out;
      }
    }
    return null;
