...
class HL7FieldData {
   ...
   */
  constructor(field, def, level, customDict, delimiters) {
  ...
  defineComponent(sequence, def) {
    ...
    const comp = new HL7FieldData(this._field, def, 1, this._customDict, this._delimiters);
  ...
  parse(hl7Text) {
    ...
    const sep = this._level ? this._delimiters[3] : this._delimiters[2];
  ...
  view() {
    ...
    if (this._items) {
      for (const f of this._items) {
        const name = f._def.desc.replace(/[^\w]/g, '');
        out[name] = f.view();
      }
      return out;
    }
